            ...
            {otherCases.map((caseItem) => (
              <div key={caseItem.id} className="group">
                <Link to={`/cases/${caseItem.id}`} className="block h-full">
                  <div className="bg-white rounded-xl overflow-hidden border border-bagh-100 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-[1.02] h-full flex flex-col">
                    {/* Image */}
                    <div className="relative flex-shrink-0">
                      ...
                      <div className="absolute top-4 left-4">
                        <span className="inline-block bg-white/95 text-bagh-800 text-xs-robotic font-light px-3 py-1.5 rounded-lg shadow-sm">
                          {caseItem.category}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-6 space-y-4 flex-1 flex flex-col">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {caseItem.technologies.slice(0, 3).map((tech, techIndex) => (
                          <span
                            key={techIndex}
                            className="bg-bagh-50 text-bagh-700 px-3 py-1 rounded-lg text-xs-robotic font-light border border-bagh-200"
                          >
                            {tech}
                          </span>
                        ))}
                      ...
                      <div className="flex items-center text-bagh-600 group-hover:text-bagh-700 transition-colors duration-200 mt-auto">
                        <span className="text-sm-robotic font-light">View Case Study</span>
